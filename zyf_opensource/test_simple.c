...
uint32_t get_new_input_conv(int m_n, int r, int c, volatile uint32_t * const dcache, uint8_t instr) {
    ...
    for (int i = 0; i < 3; i++) {
        new_input |= (get_val_conv(m_n + i, r, c + i, dcache) << ((2 - i) * 8));
    }
    return new_input;
}
...
void store_conv2_output(volatile uint32_t * const dcache, int num_outputs, uint32_t out) {
    ...
    volatile uint32_t * fc_input_addr = dcache + offset;
...
int main(void) {
    ...
    volatile uint32_t *       t_conv1    = (volatile uint32_t *)(DCACHE_BASE_ADDR + FIX_SPACE_1*4);
    volatile uint32_t *       t_conv2    = (volatile uint32_t *)(DCACHE_BASE_ADDR + FIX_SPACE_2*4);
    ...
    // 几个关键参数：m_n, r, c. 对应的4bytes 地址偏移为 =（39 * c + 3 * r）+ [m_n / 4]
    for (int c = 0; c < conv2_input_1; c++) {
    ...
    *acc0 = *src;
    ...
    for (int i = 0; i < 3; i++) {
        // 取src的高8位作为instr
        uint8_t instr = (uint8_t)((*src >> 24) & 0xFF);
        ++src;
        uint32_t new_input = get_new_input_fc(i, t_conv2, instr);
        *acc0 = new_input;
        ++t_conv2;
    }
    ...
    *acc0 = 0x0C000000;
    ...
    uint8_t final_result = (uint8_t)((*acc0 >> 8) & 0xFF);
