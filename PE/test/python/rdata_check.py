...
out_conv2_py = torch.zeros((1, 10, 12, 11), dtype=torch.uint8)
for i in range(10):
    out_conv2_py[:, i:i+1, :, :] = quantized_conv2d(q_conv2_input[:, i:i+1, :, :], q_conv2_w[i:i+1, :, :, :], q_conv2_b[i:i+1], relu=False)

# print(out_conv2_py[:, 0:1, :, :])
...
out_conv1_npu = load_out_npu(data, 0, 546, 1, 10, 14, 13)
out_conv2_npu = load_out_npu(data, 546, 942, 1, 10, 12, 11)

# print(out_conv2_npu[:, 0:1, :, :])
