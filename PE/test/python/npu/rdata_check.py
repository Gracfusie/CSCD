...
def print_output(f, name, q):
    f.write(name+':\n')
    if len(q.shape) == 2:
        for i in range(q.shape[0]):
            for j in range(q.shape[1]):
                f.write(format(q[i, j], '02x')+' ')
            f.write('\n')
        f.write('\n')
    elif len(q.shape) == 3:
        C, H, W = q.shape
        for c in range(1):
            for h in range(H):
                for w in range(W):
                    f.write(format(q[c, h, w]+256, '02x')+' ')
                f.write('\n')
            f.write('\n')
        f.write('\n')
    else:
...
# save the outputs to a file for debugging
torch.save({
    'input': q_sample_input,
    'output_conv1': out_conv1_py,
    'output_conv2_im': out_conv2_im_py,
    'output_conv2': out_conv2_py,
    'output_fc1': out_fc1_py,
    'output_fc2': out_fc2_py
}, '../output_py.pt')

with open('../output_py.txt', 'w') as f:
    print_output(f, 'input', q_sample_input)
    print_output(f, 'output_conv1', out_conv1_py)
    print_output(f, 'output_conv2_im', out_conv2_im_py)
    print_output(f, 'output_conv2', out_conv2_py)
    print_output(f, 'output_fc1', out_fc1_py)
    print_output(f, 'output_fc2', out_fc2_py)